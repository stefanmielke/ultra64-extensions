...
void tween_tick(Tween *tween) {
	...
	u64 current_ms = get_ticks_ms();
	u64 tick_since_last = current_ms - tween->last_ms;
...
void tween_start(Tween *tween, void *target_object, fnTWEasingFunction easing_function,
				 ...
				 bool always_repeat) {
	tween->type = TWEEN_NONE;
	tween_restart(tween, target_object, easing_function, duration_in_ms, tween_ending, auto_reverse,
				  always_repeat);
}

void tween_restart(Tween *tween, void *target_object, fnTWEasingFunction easing_function,
				   uint64_t duration_in_ms, fnTWCallbackEnding tween_ending, bool auto_reverse,
				   bool always_repeat) {
...
#define GET_OR_ALLOC_TWEEN_VAR(struct_type, tween_type)                                            \
	(tween->type == tween_type) ? tween->tween_values                                              \
								: MEM_ALLOC(sizeof(struct_type), tween->allocator);
...
void tween_set_to_float(Tween *tween, float start_value, float end_value,
						fnTWCallbackFloat tween_callback) {
	TweenValuesFloat *values = GET_OR_ALLOC_TWEEN_VAR(TweenValuesFloat, TWEEN_FLOAT);
...
void tween_size_tick(Tween *tween, float tick_diff) {
	...
	Size new_size = {next_value_width, next_value_height};
	values->tween_callback(tween->target_object, new_size);
}
...
void tween_set_to_position(Tween *tween, Position start_value, Position end_value,
						   fnTWCallbackPosition tween_callback) {
	TweenValuesPosition *values = GET_OR_ALLOC_TWEEN_VAR(TweenValuesPosition, TWEEN_POSITION);
...
void tween_position_tick(Tween *tween, float tick_diff) {
	...
	values->tween_callback(tween->target_object, new_position);
}
